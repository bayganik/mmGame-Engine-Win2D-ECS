...
namespace mmGameEngine
{
    public static class Mathf
    {
        ...
        {
            float num = Repeat(b - a, (float)MathHelper.TwoPi);
            if (num > MathHelper.Pi)
                num -= (float)MathHelper.TwoPi;
        ...
        {
            var num = Repeat(target - current, (float)(2 * MathHelper.Pi));
            if (num > MathHelper.Pi)
                num -= (float)(2 * MathHelper.Pi);
        ...
        public static float ApproachAngleRadians(float start, float end, float shift)
        {
            ...
            return Repeat(Approach(start, start + deltaAngleRadians, shift), (float)MathHelper.TwoPi);
        }
        ...
        //	var damped = Pow((float)MathHelper.E, -damping * wrappedTime);
